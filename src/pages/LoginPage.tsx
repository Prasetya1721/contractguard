...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // Simulasi autentikasi (demo mode)
    ...
    toast.success(mode === 'register' ? 'Akun berhasil dibuat! Selamat datang di ContractGuard.' : 'Berhasil masuk!');
    setLoading(false);
    navigate('/app');
  };
  ...
  const handleDemoLogin = async () => {
    ...
    toast.success('Masuk sebagai Demo User!');
    setLoading(false);
    navigate('/app');
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex justify-center pt-10 pb-6">
        <Link to="/" className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-gray-900 text-xl">ContractGuard</span>
        </Link>
      </div>
      ...
      <div className="flex-1 flex items-start justify-center px-4 pt-4">
        <div className="w-full max-w-md card p-8">
          ...
          <p className="text-sm text-gray-500 mb-6">
            {mode === 'login'
              ? 'Masuk untuk melanjutkan review kontrak Anda.'
              : 'Daftar gratis, 3 analisis/bulan tanpa kartu kredit.'}
          </p>
          ...
          <button
            ...
            disabled={loading}
          >
            🎯 Masuk sebagai Demo (Tanpa Registrasi)
          </button>
